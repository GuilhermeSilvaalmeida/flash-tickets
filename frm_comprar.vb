...
Public Class frm_comprar
    ...
    Private Sub btn_comprar_Click(sender As Object, e As EventArgs) Handles btn_comprar.Click
        ...
        End If
        Try
            sql = $"INSERT INTO tb_compra (usuario_comprador, id_jogo, data_compra, metodo_pagamento, valor_compra) VALUES ('{sessao_usuario}', '{id_jogos}', {data_atual}, '{metodo_pagamento}', '{valor_total}')"
            rs = db.Execute(sql)
        Catch ex As Exception
            MsgBox($"erro '{sessao_usuario}', '{id_jogos}', '{data_atual}', '{metodo_pagamento}', '{valor_total}'")
        End Try
    End Sub
